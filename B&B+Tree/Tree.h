...
protected:
	...
	/**
	 * @brief delete a value from btree
	 ...
	 * @param root: root of tree
	 */
	 virtual bool btree_query(btree_node *root, int target, int query_counts)=0;
	 /**
      * @brief target to query in the btree
	  *
	  * @param root: root of tree
	  * @param target: target to query
	  * @param query_counts: times to query
	  *
	  * @return: bool to find or not
	  */
...
public:
	...
	*/
	...
	{
        return btree_query(roots,target,query_counts);
	}
